...
public class FrequencyMap {
    ...
    public static void main(String[] args) {
        ...
        nums.stream().filter(num -> !dublicate.add(num)).forEach(System.out::println);
        ...
        nums.stream().map(x -> x*x).filter(x -> x>50).forEach(System.out::println);
       ...
       System.out.println("25 new Question is to find the dublicate Element in list");
        List<Integer> num2 = Arrays.asList(1,2,3,4,5,1,2,6);
        num2.stream()
        ...
        System.out.println("26 new question Find the first non-repeated character in a string");
        String input = "swiss";
...
| Feature       | `Optional.of()`                   | `Optional.ofNullable()`    |
        | ------------- | --------------------------------- | -------------------------- |
        | Null allowed? | ❌ No                              | ✅ Yes                      |
        | Null handling | Throws `NullPointerException`     | Returns `Optional.empty()` |
        | Use case      | When value is guaranteed non-null | When value may be null     |
